...
def read_csv_to_dict(content: bytes, header=True, regex_string=r'"([^"]*)"|([^,]+)'):
    ...
    if header:
        column_header = [x[0] or x[1] for x in re.findall(regex_string, row_datas[0])]
        row_datas = row_datas[1:]
    else:
        column_header = list(
            range(len([x[0] or x[1] for x in re.findall(regex_string, row_datas[0])]))
        )
    data = []
    logging.debug("Converting string content to dictionary")
    for row_data in row_datas:
        if row_data:
            row_content = [x[0] or x[1] for x in re.findall(regex_string, row_data)]
...
def dict_to_csv(dict_data, column, index_name = "ID"):
    if index_name:
        output_csv_content = f"{index_name}," + ",".join(column) + "\n"
    else:
